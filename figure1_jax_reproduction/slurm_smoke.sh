#!/usr/bin/env bash
#SBATCH --job-name=fig1-jax-smoke
...
#SBATCH --output=figure1_jax_reproduction/results_fasrc_jax_smoke/slurm-%j.log

set -euo pipefail

PROJECT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
OUT="$PROJECT/results_fasrc_jax_smoke"
...

{
  ...
  echo "command=$PYTHON reproduce_jax.py --data $DATA --out $OUT --device cuda --widths 8 16 --steps 8 --snapshots 3 --hessian-iters 2 --subset-size 512 --skip-sweep"
  nvidia-smi
