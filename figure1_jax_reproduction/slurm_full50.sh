#!/usr/bin/env bash
#SBATCH --job-name=fig1-jax-full50
...
#SBATCH --output=figure1_jax_reproduction/results_fasrc_jax_full50/slurm-%j.log

set -euo pipefail

PROJECT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
OUT="$PROJECT/results_fasrc_jax_full50"
...

{
  ...
  echo "command=$PYTHON reproduce_jax.py --data $DATA --out $OUT --device cuda --subset-size 50000 --widths 64 256 1024 --steps 19550 --snapshots 51 --hessian-iters 10 --skip-sweep"
  nvidia-smi
